...
<div class="col-md-12">
  <div class="box">
    <div class="box-header with-border">
      <h3 class="box-title">Detalles de Reservación de Eventos</h3>
    </div>
    <!-- /.box-header -->
    <div class="box-body">
      ...
      foreach($records as $rec) {
        ...
        <tr>
          ...
          <td><?php if($status == "PENDING") {?>
            <a href="javascript:approve('<?php echo $user_id ?>');">Aprobar</a>&nbsp;/
            &nbsp;<a href="javascript:decline('<?php echo $user_id ?>');">Rechazar</a>&nbsp;/
            &nbsp;<a href="javascript:deleteUser('<?php echo $user_id ?>');">Eliminar</a>
            <?php } else { ?>
            <a href="javascript:deleteUser('<?php echo $user_id ?>');">Eliminar</a>
            <?php }//else ?>
          </td>
...
function approve(userId) {
    if(confirm('¿Está seguro de que desea aprobarlo?')) {
...
function decline(userId) {
    if(confirm('¿Está seguro de que desea rechazar la reserva?')) {
        window.location.href = '<?php echo WEB_ROOT; ?>api/process.php?cmd=regConfirm&action=denide&userId='+userId;
    }
}
function deleteUser(userId) {
    if(confirm('Eliminar el usuario también eliminará su reserva del calendario.\n\n¿Está seguro de que desea continuar?')) {
